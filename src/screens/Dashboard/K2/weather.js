...
const Weather = () => {
  ...
  const fetchWeatherData = async () => {
    try {
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=35.237499&lon=74.589165&appid=0403d50dced9f9ec8209054dd41f9cfa`
      );
  ...
  // Check if weatherData and the required properties are available
  if (!weatherData || !weatherData.main || !weatherData.weather) {
    return <Text>Error: Weather data not available.</Text>;
  }
  ...
  return (
    <ScrollView style={{ padding: 10 }}>
      ...
      <Card style={{ marginBottom: 20, padding: 10 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: 30 }}>
          <View>
            <Text>Min: {Math.round(weatherData.main.temp_min - 273.15)}°C</Text>
            <Text>Max: {Math.round(weatherData.main.temp_max - 273.15)}°C</Text>
            <Text>{new Date(weatherData.dt * 1000).toLocaleDateString()}</Text>
          </View>
          <View style={{ justifyContent: "center", marginHorizontal: 40, marginTop: -130 }}>
            <Image source={require('../../../assets/icons/wind.png')} style={{ width: 90, height: 60 }} />
